...
parser.add_argument('--num_alphas', type=int, default=20, help='Number of Lasso penalty increments')
...
tips = pd.DataFrame()
tips["prediction"] = pd.Series(np.ravel(result).astype(float)) 
tips["original_data"] = pd.Series(np.ravel(Y_test).astype(float)) 
...
with mlflow.start_run(run_name = "Final Model - Test Data", nested = True):
    # log informational log files
    mlflow.log_artifact('polynomial_regression.txt', 'logs')

    # log input parameters
    mlflow.log_param('alpha', parameters[0]/10)
    mlflow.log_param('order', parameters[1])

    # log performance plot
    mlflow.log_artifact('model_performance_test.jpg', 'plots')

    # log mse and average delay in minutes
